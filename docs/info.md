...
| 2 | voter_id[15:8] | voter_id[19:16] shifted left 4 OR 0b0000_0110 | Latch upper byte and nibble |
...
|------|--------|---------------|
| test_valid_ids | MOD-02 | All 8 registered IDs return id_valid=1 |
| test_invalid_ids | MOD-02 | 5 unregistered IDs return id_valid=0 |
...
2. Load a voter ID using the two byte-write protocol (see pin table above).
3. Pulse data_ready (uio_in[3]) high for exactly one clock cycle.
4. Wait 2 clock cycles.
5. Read uo_out[0] (id_valid): 1 means registered voter, 0 means unregistered.
